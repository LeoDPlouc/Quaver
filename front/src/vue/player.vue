...
export default defineComponent({
  mounted() {
    setInterval(() => {
      try {
        this.progress =
          (100 * (this.player as Howl).seek()) / (this.player as Howl).duration();
      } catch {}
    });
  },
  data() {
  ...
  methods: {
    play(event: Event) {
      try {
        let player = this.player as Howl;
    ...
    changeProgress(e: Event) {
      try {
        (this.player as Howl).seek(
          (this.progress / 100) * (this.player as Howl).duration()
        );
      } catch {}
    },
  },
