...
@injectable()
@registry([{
  token: ImageFileServiceToken,
  useClass: ImageFileServiceImpl
}])
export class ImageFileServiceImpl implements ImageFileService {
  public async saveImageFileToDisk(data: string, extension: string): Promise<string> {
  ...
  constructor(@inject(PathServiceToken) private pathService: PathService) { }
}
